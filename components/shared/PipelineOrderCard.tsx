...
function OrderCard({ o, groups, phases, items, isOpen, onToggleOpen, bulkMode, selectedItems,
  ...
  isSelected, onToggleOrder, onOpenDetail, accentColor, onImageClick, waMessage, onWaClick }: {
  ...
  waMessage?: string; onWaClick?: (o: PipelineOrder) => void;
}) {
  const wp = o.customerPhone ? waPhone(o.customerPhone) : '';
  const waHref = wp
    ? `https://wa.me/${wp}${waMessage ? `?text=${encodeURIComponent(waMessage)}` : ''}`
    : '';
    ...
    <div className={`poc-card${isSelected ? ' selected' : ''}`}>
      <div className="poc-header" onClick={onToggleOpen}>
        ...
        {wp && (
          <>
            {onWaClick ? (
              <button type="button" className="poc-wa" title="WhatsApp"
                onClick={e => { e.stopPropagation(); onWaClick(o); }}>
                {WA_ICON}
              </button>
            ) : (
              <a href={waHref} className="poc-wa" onClick={e => e.stopPropagation()} target="_blank" rel="noreferrer" title="WhatsApp">
                {WA_ICON}
              </a>
            )}
...
export interface PipelineOrderListProps {
  ...
  onImageClick?: (li: PipelineLineItem) => void;
  waMessage?: string;
  onWaClick?: (o: PipelineOrder) => void;
}
...
export default function PipelineOrderList({
  ...
  onOpenDetail, onImageClick, waMessage, onWaClick,
